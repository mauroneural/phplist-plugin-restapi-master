...
class Subscribers
{
    ...
    /**
     * Add / Updates multiples Subscribers a una lista.
     *
     * <p><strong>Parameters:</strong><br/>
     * [*subscribers] {json}  array of object with this properties (  email, confirmed, htmlemail, foreignkey, subscribepage, password, disabled )
     * </p>
     ...
     * </p>
     */
    public static function subscriberBulkAddWithList()
    {
        $subscribers =  json_decode( stripslashes($_REQUEST['subscribers'] ), true );
        if(!isset($_REQUEST['subscribers'])){
            return Response::outputError(new \Exception("subscribers variable not set"));
        ...
        try {
            ...
            $stmtList = $db->prepare('replace into '.$GLOBALS['tables']['listuser'].' (userid,listid,entered) values(:userid,:listid,now())');
            ...
            foreach ($subscribers as $subscriber) {
                ...
                $stmtList->bindParam('userid', $memLastSubsid, PDO::PARAM_INT);
                $stmtList->bindParam('listid', $_REQUEST['list'], PDO::PARAM_INT);
                $stmtList->execute();
            }
            $db->commit();
            $response = new Response();
